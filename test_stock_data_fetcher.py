...
class TestStockDataFetcher(unittest.TestCase):
    ...
    def test_stock_code_format(self):
        ...
        if len(stocks) > 0:
            ...
            # 测试 ts_code 格式
            self.assertTrue(all(stocks['ts_code'].str.match(r'^\d{6}\.(SH|SZ)$')))
    ...
    def test_stock_data_structure(self):
        ...
        if len(stocks) > 0:
            # 测试第一个股票的数据
            test_stock = stocks.iloc[0]['ts_code']
    ...
    def test_error_handling(self):
        """测试错误处理"""
        # 测试无效的股票代码
        invalid_data = get_stock_data('000000.XX')
        self.assertIsNone(invalid_data)
            
    def test_valid_stock(self):
        """测试有效股票数据获取"""
        # 测试一个已知的股票代码
        data = get_stock_data('000001.SZ')  # 平安银行
